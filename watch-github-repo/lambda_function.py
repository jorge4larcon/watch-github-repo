...
def require_env(name: str, default: Optional[str] = None):
    """
    Retrieves an environment variable and throws an exception if it does not
    exists.
    """
    value = os.getenv(name, default)
    if not value:
        raise EnvironmentError(f'The environment variable "{name}" is '
                               'required.')
    return value
...
def lambda_handler(_event, _context):
    ...
    github_repo_files = require_env('GITHUB_REPO_FILES')
    github_repo_api_url = require_env('GITHUB_REPO_API_URL')
    s3_bucket = require_env('S3_BUCKET')
    s3_obj_key = require_env('S3_OBJ_KEY')
    telegram_chat_id = require_env('TELEGRAM_CHAT_ID')
    telegram_bot_token = require_env('TELEGRAM_BOT_TOKEN')
    project_name = require_env('PROJECT_NAME')
    files_to_watch = github_repo_files.split(',')
    watch_files(s3_bucket, s3_obj_key, github_repo_api_url, files_to_watch,
                project_name, TELEGRAM_MSG_TEMPLATE_FILE, telegram_chat_id,
                telegram_bot_token)
